...
const yargs = require('yargs')
  ...
  .option('coin', { alias: 'c', type: 'string', demandOption: true })
...
const main = async () => {
  let {
    privKey, network, pkg, rewardLedger, coin, amount
  } = yargs.argv
  ...
  console.debug('deposit data', { rewardLedger, coin, amount })

  const txb = new TransactionBlock()
  txb.moveCall({
    target: `${pkg}::reward::deposit`,
    arguments: [
  ...
  const coinObj = await suiClient.getObject({ id: coin, options: { showContent: true } })
  console.log('coin remaining bal', fromUnit(coinObj.data?.content?.fields?.balance ?? 0))
