...
const main = async () => {
  const { privKey, network, pkg, adminRole, account } = yargs.argv
  ...
  console.debug('arguments', { adminRole, account })

  const txb = new TransactionBlock()
  txb.moveCall({
    target: `${pkg}::reward::transfer_admin_role`,
    arguments: [
  ...
  const writer = await suiClient.getObject({ id: adminRole, options: { showContent: true, showOwner: true } })
  console.log('admin role', inspect(writer, false, 100, true))
}
